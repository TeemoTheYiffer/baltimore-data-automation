...
class PropertyDataAPI:
    ...
    def __init__(self, config: Optional[AppConfig] = None, county: str = "baltimore"):
        ...
        try:
            ...
            if len(data) == 0:
                ...
                fallback_response = self.session.get(
                    fallback_url, timeout=self.config.REQUEST_TIMEOUT
                )
            ...
            if len(data) == 0:
                logger.warning(
                    f"No data found for {self.county_config.identifier_type}: {original_identifier}"
                )
                return {
                    "success": False,
            ...
            logger.error(
                f"Error fetching data for identifier {original_identifier}: {e}"
            )
