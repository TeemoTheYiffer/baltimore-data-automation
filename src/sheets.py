...
class SheetsManager:
    ...
    def _get_sheet_headers(self, sheet_name: str, county_config: Optional[CountyConfig] = None) -> List[str]:
        ...
        spreadsheet_id = county_config.spreadsheet_id if county_config else self.config.SPREADSHEET_ID
        ...
        for retry in range(5):
            try:
                ...
                logger.info(f"Fetched and cached {len(headers)} headers for sheet: {sheet_name} in spreadsheet: {spreadsheet_id}")
    ...
    def get_property_identifiers(self, config, sheet_name: str, county_config: CountyConfig) -> List[tuple]:
        """
        ...
        try:
            ...
            if identifier_col_index == -1:
                raise ValueError(f"{county_config.identifier_column} column not found in headers")
            ...
            status_result = self.service.spreadsheets().values().get(
            ...
            if not status_values:
                logger.info("No status values found in range - treating all rows as needing processing")
                rows_to_process = [row for row in range(start_row, end_row + 1) if row not in rows_to_skip]
            else:
                ...
                for i, row_status in enumerate(status_values):
                    ...
                    if no_data_msg in status.lower() and not config.RETRY_FAILED_ROWS:
                        skip_row = True
                        logger.info(f"Skipping row {actual_row} with status containing '{no_data_msg}' (RETRY_FAILED_ROWS is False)")
            ...
            for i in range(0, len(rows_to_process), batch_size):
                ...
                for j, value_range in enumerate(value_ranges):
                    ...
                    if identifier:
                        ...
                        row_index = rows_to_process[i+j] - 2  # -2 to convert from 1-indexed sheet to 0-indexed program
                        identifier_data.append((row_index, identifier))
            
            logger.info(f"Retrieved {len(identifier_data)} identifiers from {sheet_name}")
    ...
    def batch_update_bill_details(self, updates, sheet_name="Water Bill"):
        ...
        try:
            ...
            if len(updates) > batch_size:
                logger.info(f"Batch size {len(updates)} exceeds max size {batch_size}, splitting into smaller batches")
                ...
                try:
                    ...
                    if len(first_half) > 10:
                        logger.info(f"Further reducing first half batch size")
                        for i in range(0, len(first_half), 10):
                            mini_batch = first_half[i:i+10]
                            try:
                                logger.info(f"Processing mini-batch of {len(mini_batch)} items")
                ...
                try:
                    ...
                    if len(second_half) > 10:
                        logger.info(f"Further reducing second half batch size")
                ...
                for retry in range(max_retries):
                    try:
                        ...
                        if retry > 0:
                            ...
                            logger.info(f"Retry {retry+1}/{max_retries}: Waiting {wait_time:.2f} seconds before retry")
                            time.sleep(wait_time)
                        ...
                        if self.tcp_manager:
                            try:
                                ...
                                return self.tcp_manager.execute_batch_with_retry(self.service, execute_batch)
                            except Exception as e:
                                logger.error(f"Failed to execute batch update after multiple retries: {e}")
                                raise
                        else:
                        ...
                        if retry == max_retries - 1:  # Last retry failed
                            logger.error("Maximum retries reached for batch update. Raising exception.")
                            raise
                        
                        # Reduce request size on connection errors by slicing the batch data
                        if len(batch_data["data"]) > 10:
                            logger.info(f"Reducing batch size from {len(batch_data['data'])} to 10 for next retry")
                            # Only keep the first items for the next retry
                            batch_data["data"] = batch_data["data"][:10]
                    except HttpError as e:
                        if e.resp.status == 429:  # Rate limit error
                            wait_time = min(60, (2 ** retry) + random.random())  # Exponential backoff with jitter
                            logger.warning(f"Rate limit exceeded. Waiting {wait_time:.2f} seconds before retry {retry+1}/{max_retries}")
                            time.sleep(wait_time)
                            if retry == max_retries - 1:  # Last retry failed
                                logger.error("Maximum retries reached for batch update. Raising exception.")
                                raise
    ...
    def _get_credentials(self) -> Credentials:
        ...
        secrets_path = os.path.normpath(os.path.join(script_dir, self.config.SERVICE_ACCOUNT_FILE))
    ...
    def _get_sheets_service(self):
        ...
        while retry_count < self.config.MAX_RETRIES:
            try:
                ...
                logger.error(f"Error building Sheets service (attempt {retry_count}/{self.config.MAX_RETRIES}): {e}")
                
                if retry_count < self.config.MAX_RETRIES:
                    logger.info(f"Retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                else:
                    logger.critical(f"Failed to build Sheets service after {self.config.MAX_RETRIES} attempts")
                    raise
        ...
        try:
            ...
            if not status_values:
                ...
                for row_num in range(start_row, end_row + 1):
                    status = status_map.get(row_num, "")  # Default to empty string if no status
            ...
            for i in range(0, len(rows_to_process), batch_size):
                ...
                address_result = self.service.spreadsheets().values().batchGet(
                    spreadsheetId=self.config.SPREADSHEET_ID,
                    ranges=batch_ranges
                ).execute()
                ...
                for j, value_range in enumerate(value_ranges):
                    row_values = value_range.get('values', [[]])[0]
                    row_index = rows_to_process[i+j] - 2  # Adjust to be 0-based with correct offset for update
            ...
            addresses = [(idx, addr) for idx, addr in addresses if (idx + start_row) not in rows_to_skip]
            ...
            if bill_data.get("success", False) and "data" in bill_data:
                ...
                for field, header in field_mapping.items():
                    try:
                        ...
                        logger.warning(f"Field '{header}' not found in headers, skipping")
                ...
                if not col_indices:
                    logger.warning(f"No valid columns to update for row {sheet_row_index}")
                    return
                ...
                try:
                    ...
                    body = {
                        "values": [[bill_data.get("message", "Error")]]
                    }
            ...
            if property_data.get("success", False) and "data" in property_data:
                ...
                for run in all_runs:
                    ...
                    if start_col == end_col:
                        ...
                        range_name = self.format_range(sheet_name, f"{start_col_letter}:{end_col_letter}", sheet_row_index)
    ...
    def batch_update_property_data(self, updates, sheet_name="LIENS"):
        ...
        try:
            ...
            updates = [(idx, data) for idx, data in updates if idx + 2 > 1]  # +2 accounts for 0-indexed to 1-indexed and header row
            if len(updates) == 0:
            ...
            # If updates list is too large, recursively split it
            if len(updates) > batch_size:
                logger.info(f"Property batch size {len(updates)} exceeds configured size {batch_size}, splitting into smaller batches")
                midpoint = len(updates) // 2
                first_half = updates[:midpoint]
                second_half = updates[midpoint:]
                
                # Process first half
                logger.info(f"Processing first half property batch ({len(first_half)} items)")
                try:
                    ...
                    # If the batch is still large, reduce further
                    if len(first_half) > 10:
                        logger.info(f"Further reducing first half property batch size")
                        for i in range(0, len(first_half), 10):
                            mini_batch = first_half[i:i+10]
                            try:
                                logger.info(f"Processing property mini-batch of {len(mini_batch)} items")
                ...
                # Process second half
                logger.info(f"Processing second half property batch ({len(second_half)} items)")
                try:
                    ...
                    # If the batch is still large, reduce further
                    if len(second_half) > 10:
                        logger.info(f"Further reducing second half property batch size")
                        for i in range(0, len(second_half), 10):
                            mini_batch = second_half[i:i+10]
                            try:
                                logger.info(f"Processing property mini-batch of {len(mini_batch)} items")
            ...
            for row_index, property_data in updates:
                ...
                status_range = self.format_range(sheet_name, status_col_letter, sheet_row_index)
            ...
            if batch_data["data"]:
                logger.info(f"Executing property batch update for {len(batch_data['data'])} ranges")
                ...
                for retry in range(max_retries):
                    try:
                        ...
                        if retry > 0:
                            wait_time = min(60, (2 ** retry) + random.random())
                            logger.info(f"Property retry {retry+1}/{max_retries}: Waiting {wait_time:.2f} seconds before retry")
                            time.sleep(wait_time)
                        ...
                        logger.info(f"Property batch update successful")
                        time.sleep(3)  # Delay after successful batch
                        return
                    except (ConnectionAbortedError, ConnectionResetError, ConnectionError) as e:
                        logger.warning(f"Connection error during property batch update (retry {retry+1}/{max_retries}): {e}")
                        
                        if retry == max_retries - 1:  # Last retry failed
                            logger.error("Maximum retries reached for property batch update.")
                            raise
                        
                        # Reduce request size on connection errors by slicing the batch data
                        if len(batch_data["data"]) > 10:
                            logger.info(f"Reducing property batch size from {len(batch_data['data'])} to 10 for next retry")
                            # Only keep the first items for the next retry
                            batch_data["data"] = batch_data["data"][:10]
                    except HttpError as e:
                        if e.resp.status == 429:  # Rate limit error
                            wait_time = min(60, (2 ** retry) + random.random())
                            logger.warning(f"Rate limit exceeded. Waiting {wait_time:.2f}s before property retry {retry+1}/{max_retries}")
                            time.sleep(wait_time)
                            if retry == max_retries - 1:  # Last retry failed
                                logger.error("Maximum retries reached for property batch update.")
                                raise
                        else:
                            logger.error(f"HTTP error during property batch update: {e}")
                            raise
                    except Exception as e:
                        logger.error(f"Unexpected error during property batch update: {e}")
                        raise
