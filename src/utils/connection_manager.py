...
class TCPConnectionManager:
    ...
    def apply_optimizations(self):
        ...
        logger.info(
            f"Increased socket timeout from {original_timeout} to {self.settings.TCP_TIMEOUT} seconds"
        )
        ...
        if self.is_windows:
            try:
                ...
                if find_spec("win_inet_pton") is not None:
                    logger.info("win_inet_pton imported successfully")
                else:
                    logger.warning(
                        "win_inet_pton not available - some IPv6 optimizations won't be applied"
                    )
                ...
                logger.error(
                    f"Failed to apply some Windows-specific TCP optimizations: {e}"
                )
        ...
        for retry in range(max_retries):
            try:
                ...
                if retry > 0:
                    ...
                    logger.info(
                        f"Retry {retry}/{max_retries} for batch update. Waiting {wait_time:.2f} seconds"
                    )
                    time.sleep(wait_time)

                # Execute the batch update
                logger.info(
                    f"Executing batch update (attempt {retry + 1}/{max_retries})"
                )
                ...
                if retry == max_retries - 1:
                    logger.error(
                        f"Failed to execute batch update after {max_retries} retries"
                    )
                    raise
                ...
                if e.resp.status == 429:
                    ...
                    logger.warning(
                        f"Rate limit exceeded (retry {retry + 1}/{max_retries}). Waiting {wait_time} seconds"
                    )
                    time.sleep(wait_time)
                    ...
                    if retry == max_retries - 1:
                        logger.error(
                            f"Failed to execute batch update after {max_retries} retries due to rate limiting"
                        )
                        raise
                else:
